...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/13 23:04:06 by aappleto          #+#    #+#             */
/*   Updated: 2022/11/13 23:04:10 by aappleto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_be(char **map, t_vars mlx, t_field imgs)
{
	t_loc	loc;
	...
	while (map[++loc.x])
	{
		...
		while (map[loc.x][++loc.y])
		{
			...
			{
				if (loc.x > 0 && !w(map[loc.x - 1][loc.y]))
					put_img2(mlx, imgs.edge_bot.img, loc.x * 32, loc.y * 32);
			}
		}
	}
}
